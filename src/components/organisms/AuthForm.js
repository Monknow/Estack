...
import InputLogin from "../molecules/InputLogin";
...
const MensajeDeError = styled.p`
    ...
    background-color: ${(props) =>
        props.estadoMensaje === 0 ? "rgba(59, 40, 204, 0.5)" : "#f4a4ab"};
...
const AuthForm = ({ titulo, manejarSubmit, tipo }) => {
    ...
    const emailDeURL = parametrosURl.get("email")
        ? parametrosURl.get("email")
        : "";
    const mensajeDeURL = parametrosURl.get("message")
        ? parametrosURl.get("message")
        : "";
    ...
    const elevarUsername = (usernameInput) => {
        setUsername(usernameInput);
    };

    const elevarNombre = (nombreInput) => {
        setNombre(nombreInput);
    };

    const elevarEmail = (emailInput) => {
        setEmail(emailInput);
    };

    const elevarContraseña = (contraseñaInput) => {
        setContraseña(contraseñaInput);
    };

    const levantarMensajeDeError = (mensajeCreado) => {
        setMensajeDeError(mensajeCreado);
    };

    const levantarCargando = (cargandoRecibido) => {
        setCargando(cargandoRecibido);
    };
        ...
        signin: [
            {
                ...
                placeholder: "",
                value: username,
                elevarValorInput: elevarUsername,
                type: "text",
            },
            {
            ...
            },
            {
                ...
                placeholder: "",
                value: contraseña,
                elevarValorInput: elevarContraseña,
                type: "password",
            },
        ],
        login: [
